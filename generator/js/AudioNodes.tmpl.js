...
${(function() {
  // import either BaseAudioContext or AudioContext
  const arg = d.constructor(d.node).arguments[0];
  const argType = d.memberType(arg);

    return `
import { ${argType} } from './${argType}.js';
    `;
}())}
/* eslint-enable no-unused-vars */

import { ${d.parent(d.node)} } from './${d.parent(d.node)}.js';

export class ${d.name(d.node)} extends ${d.parent(d.node)} {
  ...
  constructor(context, options) {
    ...
    parsedOptions.${optionName} = options.${optionName};
          ...
          case 'PeriodicWave': {
            checkMember += `
    if (options && options.${optionName} !== undefined) {
      if (!(options.${optionName} instanceof PeriodicWave)) {
    ...
    if (options && options.${optionName} !== undefined) {
      if (options.${optionName} !== null) {
        if (!(options.${optionName} instanceof AudioBuffer)) {
    ...
    ${d.audioParams(d.node).map(param => {
      return `
    this.#${d.name(param)} = new AudioParam({
  ...
  ${d.name(method)}(${args.map(arg => arg.optional ? `${arg.name} = ${arg.default !== null ? arg.default.value : 'null'}` : arg.name).join(', ')}) {
    ...
    if (!(${name} instanceof ${type})) {
      throw new TypeError(\`Failed to execute '${d.name(method)}' on '${d.name(d.node)}': Parameter ${index + 1} is not of type '${type}'\`);
        ...
        case 'PeriodicWave': {
          argCheck += `
    if (!(${name} instanceof PeriodicWave)) {
